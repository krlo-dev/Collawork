...
export function ProfileCard({ user }: { user: User }) {
  return (
    <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-3 hover:shadow-sm transition-shadow flex flex-col">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <h3 className="text-body-lg font-semibold text-on-surface">{user.name}</h3>
      ...
      {user.bio && <p className="text-body-md text-on-surface-variant line-clamp-2 mb-3 flex-1">{user.bio}</p>}
      ...
      <Link
        ...
        className="w-full py-2 border border-outline-variant text-on-surface text-label-md text-center rounded hover:bg-surface-container-low transition-colors"
      >
        View Profile
      </Link>
    </div>
